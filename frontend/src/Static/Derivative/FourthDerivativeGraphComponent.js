...
import '../css/FourthDerivativeGraphComponent.css';
import '../css/FourthLoadButton.css';
...
const FourthDerivativeGraphComponent = () => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${BASE_API_URL}api/fourth_derivative_graph/`, postData);
    ...
    const onSave = async () => {
        try {
            const postData = { file_path: savedFilePath };
            const response = await axios.post(`${BASE_API_URL}api/save_fourth_derivative_data/`, postData);
            if (response.data && response.data.download_url) {
                const downloadUrl = `${BASE_API_URL}${response.data.download_url}`;
                ...
                saveAs(blob, 'fourth_derivative_data.xlsx');
            }
        } catch (error) {
            console.error("Error:", error);
