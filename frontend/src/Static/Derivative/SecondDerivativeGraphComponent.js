...
const SecondDerivativeGraphComponent = () => {
    ...
    useEffect(() => {
        ...
        const fetchSavedFilePath = async () => {
            try {
                console.log("Fetching saved file path...");
                ...
                if (response.data && response.data.file_path) {
                    ...
                    console.log("Successfully set savedFilePath:", response.data.file_path);
                }
            } catch (error) {
        ...
        if (fileSaved) {  // ファイルが保存されたら実行
            console.log("File has been saved. Fetching latest file path...");
            fetchSavedFilePath();
            setFileSaved(false); // ファイルが取得されたらステートをリセット
        }
    }, [fileSaved]);

    const handleFileSaved = () => {
        console.log("Handle file saved is called.");
        setFileSaved(true);
    };
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${BASE_API_URL}api/second_derivative_graph/`, postData);
    ...
    const onSave = async () => {
        try {
            ...
            // まず、二次微分と保存
            await axios.post(`${BASE_API_URL}api/save_second_derivative_data/`, postData);

            // 次に、保存したファイルをダウンロード
            const downloadUrl = `${BASE_API_URL}api/download_second_derivative_data/`;
            ...
            link.href = blobURL;
            link.download = 'second_derivative_data.xlsx';
            link.click();
        } catch (error) {
            console.error("Error:", error);
    ...
    return (
        <div className="Main-Wrapper">
            <div className="Second-Title-Button-Container">
                ...
                <button class="Second-button" type="button" onClick={handleFileSaved}>
                    <span class="Second-button__text">Load</span>
                    <span class="Second-button__icon">
