...
function DifferenceGraphComponent() {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${BASE_API_URL}api/difference_graph/`, postData);
    ...
    const onSave = async () => {
        try {
            ...
            // まず、差スペクトルデータを保存
            await axios.post(`${BASE_API_URL}api/save_difference_data/`, postData);

            // 次に、保存したファイルをダウンロード
            const downloadUrl = `${BASE_API_URL}api/download_difference_data/`;  // ダウンロードのAPIエンドポイントを適切に設定してください。
            ...
            link.href = blobURL;
            link.download = 'difference_data.xlsx';
            link.click();
        } catch (error) {
